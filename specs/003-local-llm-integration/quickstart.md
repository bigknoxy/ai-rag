...
6. Query with LLM: `curl -X POST "http://localhost:5250/api/query" -H "Content-Type: application/json" -d '{"text":"Explain RAG","useLlm":true}'`
...
## Streaming Responses
- Use `/api/query/stream` for real-time token streaming: `curl -X POST "http://localhost:5250/api/query/stream" -H "Content-Type: application/json" -d '{"text":"Explain RAG","useLlm":true}'`
